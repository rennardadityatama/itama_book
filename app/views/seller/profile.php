        ...
        <div class="col-xl-4 col-lg-5">
          <div class="card">
            ...
            <div class="card-body">
              ...
              <div class="mb-3">
                <label class="form-label f-w-500">QRIS</label><br>
                <?php if (!empty($user['qris_photo'])): ?>
                  <img
                    src="<?= BASE_URL . '/uploads/qris/' . htmlspecialchars($user['qris_photo']) ?>"
                    alt="QRIS"
          ...
          <form class="card" method="POST" id="editProfileForm" enctype="multipart/form-data">
            ...
            <div class="card-body">
              <div class="row g-3">
                ...
                <div class="col-md-12">
                  <label class="form-label f-w-500">QRIS Image</label>
                  <input class="form-control"
                    type="file"
                    name="qris_photo"
                    id="qrisInput"
...
<script>
  ...
  const userData = {
    ...
    nik: "<?= htmlspecialchars($user['nik'], ENT_QUOTES) ?>",
    address: "<?= htmlspecialchars($user['address'], ENT_QUOTES) ?>",
    account_number: "<?= htmlspecialchars($user['account_number'] ?? '', ENT_QUOTES) ?>"
  };
  ...
  editBtn.addEventListener('click', () => {
    ...
    form.querySelector('[name="nik"]').value = userData.nik;
    form.querySelector('[name="address"]').value = userData.address;
    form.querySelector('[name="account_number"]').value = userData.account_number;
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      const res = await fetch('<?= BASE_URL ?>index.php?c=seller&m=updateProfile', {
        method: 'POST',
        body: formData
      });
